...
snpgdsBED2GDS(bed.fn=paste0(fileroot,".bed"),
              ...
              out.gdsfn="All_AD.gds",family=TRUE)

# Open .gds file in SNPRelate
genofile <- snpgdsOpen("All_AD.gds")
...
save(kinship,file="All_AD_kinship.Robj")
# Order of individuals in kinship matrix
save(samp.id,file="All_AD_kinship.sample.id.Robj")
snpgdsClose(genofile)

# Calculate PCs using GENESIS/PCAiR
kinship <- get(load("All_AD_kinship.Robj"))
kinship <- as.data.frame(kinship)
samp.id <- get(load("All_AD_kinship.sample.id.Robj"))
row.names(kinship) <- samp.id
colnames(kinship) <- samp.id 
genofile <- GdsGenotypeReader("All_AD.gds")
genodata <- GenotypeData(genofile)

# Get kinship matrix from KING
#file.king = system.file("All_AD_king_table.kin0")
#KINGmat <- kingToMatrix(
#  c(system.file("extdata", "All_AD_king_table.kin0", package="GENESIS"),
#    system.file("extdata", "All_AD.kin", package="GENESIS")),
 # estimator = "Kinship")
#KINGmat[1:5,1:5]

file.king = system.file("extdata", "All_AD_king_table.kin0", package = "GENESIS")
KINGmat <- kingToMatrix("All_AD_king.kin", estimator = "Kinship")
...
write.table(pcsmat, "All_AD_PCAiR.txt",row.names=F,quote=F,sep="\t")
...
write.table(kinmat,"All_AD_PCRelate.txt.gz",row.names=F, quote=F, sep="\t")
...
save(mypcrel, file=paste0("All_AD_mypcrel.RData"))
...
# Get PC values
pcs = read.table("All_AD_PCAiR.txt", na.strings = "", header=T, stringsAsFactors = FALSE, as.is=T)
...
save(assoc, file=paste0("All_AD_GENESIS_", main_pheno, ".RData"))

write.csv(assoc, file = "All_AD_GENESIS_GWAS.csv", quote=F, row.names=F)
