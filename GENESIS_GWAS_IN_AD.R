...
snpgdsBED2GDS(bed.fn=paste0(fileroot,".bed"),
              ...
              out.gdsfn="IN_AD.gds",family=TRUE)

# Open .gds file in SNPRelate
genofile <- snpgdsOpen("IN_AD.gds")
...
save(kinship,file="IN_AD_kinship.Robj")
# Order of individuals in kinship matrix
save(samp.id,file="IN_AD_kinship.sample.id.Robj")
snpgdsClose(genofile)

# Calculate PCs using GENESIS/PCAiR
kinship <- get(load("IN_AD_kinship.Robj"))
kinship <- as.data.frame(kinship)
samp.id <- get(load("IN_AD_kinship.sample.id.Robj"))
row.names(kinship) <- samp.id
colnames(kinship) <- samp.id 
genofile <- GdsGenotypeReader("IN_AD.gds")
genodata <- GenotypeData(genofile)

# Get kinship matrix from KING
#file.king = system.file("IN_AD_king_table.kin0")
#KINGmat <- kingToMatrix(
#  c(system.file("extdata", "IN_AD_king_table.kin0", package="GENESIS"),
#    system.file("extdata", "IN_AD.kin", package="GENESIS")),
 # estimator = "Kinship")
#KINGmat[1:5,1:5]

file.king = system.file("extdata", "IN_AD_king_table.kin0", package = "GENESIS")
KINGmat <- kingToMatrix("IN_AD_king.kin", estimator = "Kinship")
...
write.table(pcsmat, "IN_AD_PCAiR.txt",row.names=F,quote=F,sep="\t")
...
write.table(kinmat,"IN_AD_PCRelate.txt.gz",row.names=F, quote=F, sep="\t")
...
save(mypcrel, file=paste0("IN_AD_mypcrel.RData"))
...
# Get PC values
pcs = read.table("IN_AD_PCAiR.txt", na.strings = "", header=T, stringsAsFactors = FALSE, as.is=T)
...
save(assoc, file=paste0("IN_AD_GENESIS_", main_pheno, ".RData"))

write.csv(assoc, file = "IN_AD_GENESIS_GWAS.csv", quote=F, row.names=F)
