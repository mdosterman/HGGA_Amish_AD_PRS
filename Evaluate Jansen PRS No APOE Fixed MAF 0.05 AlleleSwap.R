...
OH_PRS = read.table("Jansen_Weights_OH_Test_No_APOE_MAF05.best", header=T)
IN_PRS = read.table("Jansen_Weights_IN_Test_No_APOE_MAF05.best", header=T)
...
cov_apoe_mod_OH_even  = glm(dx ~ apoe2 + apoe4 + sex + Age, data=OH_pheno_even)
...
cov_apoe_mod_IN_even  = glm(dx ~ apoe2 + apoe4 + sex + Age, data=IN_pheno_even)
...
p3=ggplot(OH_pheno, aes(x=as.factor(dx), y=PRS2, fill=as.factor(dx))) +
  geom_violin() +
  geom_boxplot(width=.2) +
  theme(legend.position = "none") +
  scale_fill_brewer(palette = "Spectral") +
  ggtitle("PRS Distributions in OH Amish") +
  theme(plot.title = element_text(hjust=0.5, size=20), axis.text = element_text(size=20), axis.title=element_text(size=20, face="bold")) +
  xlab("Dementia Status") +
  ylab("Polygenic Risk Score") +
  scale_x_discrete(labels=c("Unaffected", "Affected"))

p4=ggplot(IN_pheno, aes(x=as.factor(dx), y=PRS2, fill=as.factor(dx))) +
  geom_violin() +
  geom_boxplot(width=.2) +
  theme(legend.position = "none") +
  scale_fill_brewer(palette = "Spectral") +
  ggtitle("PRS Distributions in IN Amish") +
  theme(plot.title = element_text(hjust=0.5, size = 20), axis.text = element_text(size=20), axis.title=element_text(size=20, face="bold")) +
  xlab("Dementia Status") +
  ylab("Polygenic Risk Score") +
  scale_x_discrete(labels=c("Unaffected", "Affected"))

ggarrange(p4, p3) # Use 1000 x 400

Fig3=ggarrange(p4, p3) # Use 1500 x 500

tiff("Figure3.tiff", units = "in", width = 12, height = 6, res = 300)
Fig3
dev.off()
