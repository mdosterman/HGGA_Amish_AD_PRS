...
snpgdsBED2GDS(bed.fn=paste0(fileroot,".bed"),
              ...
              out.gdsfn="OH_CI.gds",family=TRUE)

# Open .gds file in SNPRelate
genofile <- snpgdsOpen("OH_CI.gds")
...
save(kinship,file="OH_CI_kinship.Robj")
# Order of individuals in kinship matrix
save(samp.id,file="OH_CI_kinship.sample.id.Robj")
snpgdsClose(genofile)

# Calculate PCs using GENESIS/PCAiR
kinship <- get(load("OH_CI_kinship.Robj"))
kinship <- as.data.frame(kinship)
samp.id <- get(load("OH_CI_kinship.sample.id.Robj"))
row.names(kinship) <- samp.id
colnames(kinship) <- samp.id 
genofile <- GdsGenotypeReader("OH_CI.gds")
genodata <- GenotypeData(genofile)

# Get kinship matrix from KING
#file.king = system.file("OH_CI_king_table.kin0")
#KINGmat <- kingToMatrix(
#  c(system.file("extdata", "OH_CI_king_table.kin0", package="GENESIS"),
#    system.file("extdata", "OH_CI.kin", package="GENESIS")),
 # estimator = "Kinship")
#KINGmat[1:5,1:5]

file.king = system.file("extdata", "OH_CI_king_table.kin0", package = "GENESIS")
KINGmat <- kingToMatrix("OH_CI_king.kin", estimator = "Kinship")
...
write.table(pcsmat, "OH_CI_PCAiR.txt",row.names=F,quote=F,sep="\t")
...
write.table(kinmat,"OH_CI_PCRelate.txt.gz",row.names=F, quote=F, sep="\t")
...
save(mypcrel, file=paste0("OH_CI_mypcrel.RData"))
...
# Get PC values
pcs = read.table("OH_CI_PCAiR.txt", na.strings = "", header=T, stringsAsFactors = FALSE, as.is=T)
...
save(assoc, file=paste0("OH_CI_GENESIS_", main_pheno, ".RData"))

write.csv(assoc, file = "OH_CI_GENESIS_GWAS.csv", quote=F, row.names=F)
